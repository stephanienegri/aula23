<?php
    ...
    $comandoSQL = "select turmas.numero,
                          turmas.horarioAula,
                          turmas.duracaoAula,
                          turmas.dataInicial,
                          turmas.dataFinal,
                          tiposatividade.descricao as atividade,
                          alunos.nome as alunoMonitor,
                          instrutores.nome as nomeInstrutor,
                          titulos.descricao as tituloInstrutor       
                    from turmas
                    inner join tiposatividade
                        on turmas.codigoTipoAtividade = tiposatividade.codigo
                    inner join alunos
                        on turmas.matriculaMonitor = alunos.matricula
                    inner join instrutores
                        on turmas.idInstrutor = instrutores.id
                    inner join titulos
            ...
            <table class="table
                ...
                <?php 
                    while ( $linha = mysqli_fetch_array($resultadoConsulta) ) {
                        $numero = $linha['numero'];
                        $horarioAula = $linha['horarioAula'];
                        $duracaoAula = $linha['duracaoAula'];
                        $dataInicial = $linha['dataInicial'];
                        $tipoAtividade = $linha['atividade'];
                        $nomeInstrutor = $linha['nomeInstrutor'];
                        $tituloInstutor = $linha['tituloInstrutor'];
                        $alunoMonitor = $linha['alunoMonitor'];

                        // formatando as datas para dd-mm-yyyy
                        $timestamp = strtotime($dataInicial);
                        $dataInicialFormatada = date("d/m/Y", $timestamp);
                        
                        // tratando a data final que pode ser nula
                        // porque a turma ainda esta ativa
                        if ($linha['dataFinal'] == NULL) {
                            $dataFinalFormatada = '--';
                        } else {
                            $dataFinal = $linha['dataFinal'];
                            $timestamp = strtotime($dataFinal);
                            $dataFinalFormatada = date("d/m/Y", $timestamp);
                        }
                ?>
